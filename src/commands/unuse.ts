...
export async function unuseCommand(
	...
	if (!isPackageLinked(cwd, packageName)) {
		console.log(
			chalk.yellow(`⚠ Package "${packageName}" is not linked in this project`),
		);
		return;
	}
	...
	if (!pkg) {
		...
		console.log(
			chalk.gray(
				"  The manifest may have been removed. Check .monolink-local.json",
			),
		);
		process.exit(1);
	}
	...
	try {
		...
		console.log(
			chalk.red(`✗ Failed to update package.json: ${(err as Error).message}`),
		);
		process.exit(1);
	}
