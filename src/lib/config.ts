...
/**
 * Read local monolink config
 */
function readLocalConfig(projectPath: string): LocalLinkConfig {
	const configPath = path.join(projectPath, MONOLINK_CONFIG_FILE);
	...
	try {
		...
		logError("local-config-parse", err);
		return { linkedPackages: [] };
	}
}

/**
 * Write local monolink config
 */
...
export function applyLinkConfig(
	...
	if (localConfig.linkedPackages.length === 0) {
		localConfig.originalOverrides = pkgJson.pnpm?.overrides || {};
	}
...
export function removeLinkConfig(
	...
	localConfig.linkedPackages = localConfig.linkedPackages.filter(
		(p) => p !== packageName,
	);
	...
	if (
		localConfig.addedDependencies?.includes(packageName) &&
		pkgJson.dependencies
	) {
		delete pkgJson.dependencies[packageName];
		localConfig.addedDependencies = localConfig.addedDependencies.filter(
			(d) => d !== packageName,
		);
	...
	if (pkgJson.pnpm?.overrides) {
		for (const pkgName of packagesToRemove) {
			delete pkgJson.pnpm.overrides[pkgName];
		}

		// If no more linked packages, restore original overrides
		if (localConfig.linkedPackages.length === 0) {
			pkgJson.pnpm.overrides = localConfig.originalOverrides || {};

			// Clean up empty pnpm config
			if (
				pkgJson.pnpm.overrides &&
				Object.keys(pkgJson.pnpm.overrides).length === 0
			) {
				delete pkgJson.pnpm.overrides;
			}
			if (pkgJson.pnpm && Object.keys(pkgJson.pnpm).length === 0) {
				delete pkgJson.pnpm;
			}
		}
	}
 ...
 */
export function isPackageLinked(
	projectPath: string,
	packageName: string,
): boolean {
